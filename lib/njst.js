...
var parser = require('./parser')
  ...
  , compiler = require('./compiler').CompiledTemplate;
 ...
 * @param {object} inputParams
 */
Constructor.parse = function (content, context, inputParams, callback) {
	content = content.toString();
	var parseParams;
	if ('function' === typeof context) {
		callback = context;
		inputParams = {};
	} else if('function' === typeof inputParams){
		...
		parseParams = params.get.call(this);
		parseParams = params.extend.call(this, inputParams);
	}
	var out = parser.call(this, content, context, parseParams);
	if(callback){
		if(out.status === 'error'){
			callback(new Error(out.toString()));
		} else {
			callback(null, out);
		}
	} else return out;
 ...
 * @param {object} inputParams
 */
Constructor.compiler = function (content, inputParams) {
	content = content.toString();
	inputParams = (typeof inputParams === 'object') ? inputParams : {};
	var parseParams = params.get.call(this);
	parseParams = params.extend.call(this, inputParams);
	return new compiler(content, parseParams);
};
Constructor.prototype.compiler = Constructor.compiler;
